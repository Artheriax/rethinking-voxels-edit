        ...
        for (int x0 = 0; x0 < intpointerVolumeRes; x0++) {
                for (int y0 = 0; y0 < intpointerVolumeRes; y0++) {
                        for (int z0 = 0; z0 < intpointerVolumeRes; z0++) {
                                ...
                                // Early-out: Check depth buffer first
                                uvec4 depthData = readVoxelVolume(blockCoord, 0);
                                if (depthData.x == 0) {
                                ...
                                // Optimized: Read voxel data once and check emissive flag early
                                uvec4 packedData = readVoxelVolume(blockCoord, 1);
                                if (packedData.x == 0) continue;
                                
                                // Early-out: Check emissive flag directly from packed data (bit 3 of blocktype)
                                // blocktype is in packedData.z >> 24, emissive is bit 3
                                uint blocktype = packedData.z >> 24;
                                if ((blocktype & 8u) == 0u) continue; // Not emissive, skip
                                
                                // Only unpack full data for emissive voxels
                                vxData thisVoxelData;
                                thisVoxelData.lightcol = vec3(
                                        packedData.z % 256,
                                        (packedData.z >> 8) % 256,
                                        (packedData.z >> 16) % 256
                                ) / 255.0;
                                thisVoxelData.midcoord = vec3(0.5);
                                thisVoxelData.mat = int(packedData.x % 65536);
                                thisVoxelData.lightlevel = int(packedData.x >> 16);
                                thisVoxelData.cuboid = ((blocktype >> 4) & 1u) == 1u && ((blocktype >> 2) & 1u) == 0u;
                                thisVoxelData.full = ((blocktype >> 2) & 1u) == 1u;
                                
                                if (thisVoxelData.cuboid) {
                                        thisVoxelData.lower = vec3(
                                                packedData.w % 16,
                                                (packedData.w >> 4) % 16,
                                                (packedData.w >> 8) % 16
                                        ) / 16.0;
                                        thisVoxelData.upper = (vec3(
                                                (packedData.w >> 12) % 16,
                                                (packedData.w >> 16) % 16,
                                                (packedData.w >> 20) % 16
                                        ) + 1) / 16.0;
                                } else {
                                        thisVoxelData.lower = vec3(0);
                                        thisVoxelData.upper = vec3(1);
                                }
                                
                                // Check for crossmodel/entity for midcoord
                                if (((blocktype >> 1) & 1u) == 1u || (blocktype >> 7) == 1u) {
                                        thisVoxelData.midcoord = vec3(
                                                packedData.w % 256,
                                                (packedData.w >> 8) % 256,
                                                (packedData.w >> 16) % 256
                                        ) / 256.0;
