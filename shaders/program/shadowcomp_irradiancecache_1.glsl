...
shared int lightsToCheckShared;
void main() {
        if (gl_LocalInvocationID == uvec3(0)) {
                visibility = 0;
                lightsToCheckShared = 0;
        }
        barrier();
        groupMemoryBarrier();
        ...
        ivec3 iGlobalInvocationID = 8 * iGlobalInvocationID0 + ivec3(7.9 * hash44(vec4(iGlobalInvocationID0 + 0.5, 0))); // Use constant seed
        ivec3 thisCoord = iGlobalInvocationID0 + ivec3(gl_LocalInvocationID) - 1;
        thisCoord = 8 * thisCoord + ivec3(7.9 * hash44(vec4(thisCoord + 0.5, 0))); // Use constant seed
        ...
        if (lightCount0 == 0) return;
        int lightStripLoc0 = readVolumePointer(pgc0, 5) + 1;
        
        // FIX: Check multiple lights per frame for stable visibility detection
        int lightsToCheck = min(4, lightCount0);
        if (gl_LocalInvocationID == uvec3(0)) {
                lightsToCheckShared = lightsToCheck;
        }
        barrier();
        groupMemoryBarrier();
        
        for (int i = 0; i < lightsToCheck; i++) {
                int lightNum0 = (frameCounter + i) % lightCount0;
                int lightPointer = readLightPointer(lightStripLoc0 + lightNum0);
                if (lightPointer < 0) continue;

                int lightCount = min(64, readVolumePointer(pgc, 4));
                int lightStripLoc = readVolumePointer(pgc, 5) + 1;
                int lightNum;
                for (lightNum = 0; lightNum < lightCount && readLightPointer(lightStripLoc + lightNum) != lightPointer; lightNum++);

                uvec4 occlusionData = readOcclusionVolume(oldCacheCoord);
                if (lightNum < lightCount && (occlusionData[lightNum >> 5] & (1u << (lightNum & 31))) != 0u)
        ...
        if (gl_LocalInvocationID == uvec3(1)) {
                uvec4 occlusionData = readOcclusionVolume(oldCacheCoord);
                // Adjusted threshold: all visible or none visible
                if (visibility == 0 || visibility >= lightsToCheck)
                        occlusionData[2] = 0;
                else
                        occlusionData[2] = 1;
